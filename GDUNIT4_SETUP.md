...
#### From Command Line:cd c:\Projects\reading-racer
...
```bash
# Run all validation checks (default: non-headless)
powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\gdscript-check.ps1

# Run just the Godot validation (default: non-headless)
godot --script tools/gdscript-check.ps1
